/* 
 * 参照listdetail代码，在同一个div中，两个子div分别显示 List和明细数据，两个子div相互切换
 */
...
(function($){
	...
	/**
	 * set the listdetail scrollers to show or not,
	 * dependent on the listdetail count and width
	 */
	function setScrollers(container) {
		var opts = $.data(container, 'listdetail').options;
		if (opts.tabPosition == 'left' || opts.tabPosition == 'right'){return}
		
		var header = $(container).children('div.listdetail-header');
		var tool = header.children('div.listdetail-tool');
		var sLeft = header.children('div.listdetail-scroller-left');
		var sRight = header.children('div.listdetail-scroller-right');
		var wrap = header.children('div.listdetail-wrap');
		
		// set the tool height
		tool._outerHeight(header.outerHeight() - (opts.plain ? 2 : 0));
		
		var listdetailWidth = 0;
		$('ul.listdetail li', header).each(function(){
			listdetailWidth += $(this).outerWidth(true);
		});
		var cWidth = header.width() - tool._outerWidth();
		
		if (listdetailWidth > cWidth) {
			sLeft.show();
			sRight.show();
...
//			wrap.scrollLeft(0);
	...
	function addTools(container){
		...
		if (opts.tools) {
			if (typeof opts.tools == 'string'){
				$(opts.tools).addClass('listdetail-tool').appendTo(header);
				$(opts.tools).show();
			} else {
				header.children('div.listdetail-tool').remove();
				var tools = $('<div class="listdetail-tool"></div>').appendTo(header);
	...
	function setSize(container) {
		...
		var header = $(container).children('div.listdetail-header');
		var panels = $(container).children('div.listdetail-panels');
		...
		if (opts.tabPosition == 'left' || opts.tabPosition == 'right'){
			...
			var wrap = header.find('div.listdetail-wrap');
			wrap._outerWidth(header.width());
			header.find('.listdetail')._outerWidth(wrap.width());
		} else {
			header.css('height','');
			header.find('div.listdetail-wrap').css('width','');
			header.find('.listdetail').css('width','');
	...
	/**
	 * wrap the listdetail header and body
	 */
	function wraplistdetail(container) {
		var listdetail = $.data(container, 'listdetail').listdetail;
		var cc = $(container);
		cc.addClass('listdetail-container');
		cc.wrapInner('<div class="listdetail-panels"/>');
//		$('<div class="listdetail-header">'
//				+ '<div class="listdetail-scroller-left"></div>'
//				+ '<div class="listdetail-scroller-right"></div>'
//				+ '<div class="listdetail-wrap">'
//				+ '<ul class="listdetail"></ul>'
//				+ '</div>'
//				+ '</div>').prependTo(container);
		
		cc.children('div.listdetail-panels').children('div').each(function(i){
		...
		cc.children('div.listdetail-header').find('.listdetail-scroller-left, .listdetail-scroller-right').hover(
			function(){$(this).addClass('listdetail-scroller-over');},
			function(){$(this).removeClass('listdetail-scroller-over');}
		);
	...
	function setProperties(container){
		...
		header.removeClass('listdetail-header-top listdetail-header-bottom listdetail-header-left listdetail-header-right');
		panels.removeClass('listdetail-panels-top listdetail-panels-bottom listdetail-panels-left listdetail-panels-right');
		if (opts.tabPosition == 'top'){
			header.insertBefore(panels);
		}else if (opts.tabPosition == 'bottom'){
			header.insertAfter(panels);
			header.addClass('listdetail-header-bottom');
			panels.addClass('listdetail-panels-top');
		} else if (opts.tabPosition == 'left'){
			header.addClass('listdetail-header-left');
			panels.addClass('listdetail-panels-right');
		} else if (opts.tabPosition == 'right'){
			header.addClass('listdetail-header-right');
//			header.addClass('listdetail-header-left listdetail-header-right');
			panels.addClass('listdetail-panels-left');
		}
		
		if (opts.plain == true) {
			header.addClass('listdetail-header-plain');
		} else {
			header.removeClass('listdetail-header-plain');
		}
		if (opts.border == true){
			header.removeClass('listdetail-header-noborder');
			panels.removeClass('listdetail-panels-noborder');
		} else {
			header.addClass('listdetail-header-noborder');
			panels.addClass('listdetail-panels-noborder');
		}
		
		$('.listdetail-scroller-left', header).unbind('.listdetail').bind('click.listdetail', function(){
			$(container).listdetail('scrollBy', -opts.scrollIncrement);
		});
		
		$('.listdetail-scroller-right', header).unbind('.listdetail').bind('click.listdetail', function(){
			$(container).listdetail('scrollBy', opts.scrollIncrement);
		});
	}
	
	function createTab(container, pp, options) {
		var state = $.data(container, 'listdetail');
		options = options || {};
		...
		var listdetail = $(container).children('div.listdetail-header').find('ul.listdetail');
		
		opts.tab = $('<li></li>').appendTo(listdetail);	// set the tab object in panel options
//		opts.tab.append(
//				'<a href="javascript:void(0)" class="listdetail-inner">' +
//				'<span class="listdetail-title"></span>' +
//				'<span class="listdetail-icon"></span>' +
//				'</a>'
//		);
		opts.tab.unbind('.listdetail').bind('click.listdetail', {p:pp}, function(e){
			if ($(this).hasClass('listdetail-disabled')){return;}
			selectTab(container, getTabIndex(container, e.data.p));
		}).bind('contextmenu.listdetail', {p:pp}, function(e){
			if ($(this).hasClass('listdetail-disabled')){return;}
			state.options.onContextMenu.call(container, e, $(this).find('span.listdetail-title').html(), getTabIndex(container, e.data.p));
		});
...
//		});
	...
	function addTab(container, options) {
		...
		var pp = $('<div></div>').appendTo($(container).children('div.listdetail-panels'));
		listdetail.push(pp);
		createTab(container, pp, options);
		
//		opts.onAdd.call(container, options.title, listdetail.length-1);
		
		setScrollers(container);
		if (options.selected){
			selectTab(container, listdetail.length-1);	// select the added tab panel
		}
	}
	...
	function updateTab(container, param){
		var selectHis = $.data(container, 'listdetail').selectHis;
...
//		var s_title = tab.find('span.listdetail-title');
//		var s_icon = tab.find('span.listdetail-icon');
//		s_title.html(opts.title);
//		s_icon.attr('class', 'listdetail-icon');
		
		tab.find('a.listdetail-close').remove();
		if (opts.closable){
//			s_title.addClass('listdetail-closable');
			var a_close = $('<a href="javascript:void(0)" class="listdetail-close"></a>').appendTo(tab);
			a_close.bind('click.listdetail', {p:pp}, function(e){
				if ($(this).parent().hasClass('listdetail-disabled')){return;}
				closeTab(container, getTabIndex(container, e.data.p));
				return false;
			});
		} else{
//			s_title.removeClass('listdetail-closable');
		}
//		if (opts.iconCls){
//			s_title.addClass('listdetail-with-icon');
//			s_icon.addClass(opts.iconCls);
//		} else {
//			s_title.removeClass('listdetail-with-icon');
//		}
//		
...
//		}
		...
		tab.find('span.listdetail-p-tool').remove();
		if (opts.tools){
			var p_tool = $('<span class="listdetail-p-tool"></span>').insertAfter(tab.find('a.listdetail-inner'));
			if (typeof opts.tools == 'string'){
...
//						t.bind('click', eval(opts.tools[i].handler));
						t.bind('click', {handler:opts.tools[i].handler}, function(e){
							if ($(this).parents('li').hasClass('listdetail-disabled')){return;}
...
//			s_title.css('padding-right', pr+'px');
		...
		$.data(container, 'listdetail').options.onUpdate.call(container, opts.title, getTabIndex(container, pp));
	}
...
//		}
	...
	function getTab(container, which, removeit){
		...
		if (typeof which == 'number'){
			if (which < 0 || which >= listdetail.length){
				return null;
			} else {
				var tab = listdetail[which];
				if (removeit) {
					listdetail.splice(which, 1);
				}
				return tab;
			}
		}
		for(var i=0; i<listdetail.length; i++){
			var tab = listdetail[i];
	...
	function doFirstSelect(container){
		var listdetail = $.data(container, 'listdetail').listdetail;
		for(var i=0; i<listdetail.length; i++){
			if (listdetail[i].panel('options').selected){
	...
	function selectTab(container, which){
		...
		if (selected){
			...
			selected.panel('options').tab.removeClass('listdetail-selected');
		}
		...
		tab.addClass('listdetail-selected');
		
		// scroll the tab to center position if required.
		var wrap = $(container).find('>div.listdetail-header>div.listdetail-wrap');
//		var left = tab.position().left;
//		var right = left + tab.outerWidth();
//		if (left < 0 || right > wrap.width()){
//			var deltaX = left - (wrap.width()-tab.width()) / 2;
//			$(container).listdetail('scrollBy', deltaX);
//		} else {
			$(container).listdetail('scrollBy', 0);
//		}
	...
	$.fn.listdetail = function(options, param){
		if (typeof options == 'string') {
			return $.fn.listdetail.methods[options](this, param);
		}
		
		options = options || {};
		return this.each(function(){
			var state = $.data(this, 'listdetail');
			var opts;
			if (state) {
				opts = $.extend(state.options, options);
				state.options = opts;
			} else {
				$.data(this, 'listdetail', {
					options: $.extend({},$.fn.listdetail.defaults, $.fn.listdetail.parseOptions(this), options),
					listdetail: [],
					selectHis: []
				});
				wraplistdetail(this);
	...
	$.fn.listdetail.methods = {
		options: function(jq){
			return $.data(jq[0], 'listdetail').options;
		},
		listdetail: function(jq){
			return $.data(jq[0], 'listdetail').listdetail;
		},
		resize: function(jq){
		...
		enableTab: function(jq, which){
			return jq.each(function(){
				$(this).listdetail('getTab', which).panel('options').tab.removeClass('listdetail-disabled');
			});
		},
		disableTab: function(jq, which){
			return jq.each(function(){
				$(this).listdetail('getTab', which).panel('options').tab.addClass('listdetail-disabled');
			});
		},
		scrollBy: function(jq, deltaX){	// scroll the tab header by the specified amount of pixels
			return jq.each(function(){
				var opts = $(this).listdetail('options');
				var wrap = $(this).find('>div.listdetail-header>div.listdetail-wrap');
	...
	$.fn.listdetail.parseOptions = function(target){
