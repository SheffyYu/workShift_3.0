...
$.extend($.fn.datagrid.methods, {
    ...
    //编辑状态下动态设置某列的editor
    //datagrid.datagrid('setEditorWhenRowEditing', {index: xxx, field: xxx, editor: xxx});
        ...
        cmenu.menu({
            onClick: function(item) {
                if (item.name == 'exportExcel') {
                    dg.datagrid("hdExportExcel", {url: dg.datagrid('options').url.replace("/find", "/exportExcel"), exportFileName: (dg.datagrid('options').exportFileName ? (dg.datagrid('options').exportFileName + "-" + new Date().format("yyyyMMddhhmmss")) : ($('#main').tabs('getSelected').panel('options').title + "-" + new Date().format("yyyyMMddhhmmss")))});
                }
            }
        });
        cmenu.menu('show', {
...
$.extend($.fn.datagrid.defaults.editors, {
   uppercasebox: {
	    init: function (container, options) {
	      ...
	      if (!options.width) {
	        options.width = container.context.offsetWidth - 3;//fix prefect to -3
	      }
...
//            $(target).combogrid("setText", text);
        ...
        getValue: function(target) {
            if ($(target).combogrid('grid').datagrid('options').mapping != undefined && 
                $(target).combogrid('grid').datagrid('options').mapping.isObject != undefined) {
        ...
        destroy: function(target) {
        	...
        	return;
        	//以下代码在双击编辑时，第一次保存报错，第二次保存正常，原因不明，暂时不调用以下代码 update by huxj 2016-10-12
...
////            alert($(target).combogrid('isValid') + "-" + $(target).combogrid('getText'));
//            }
        ...
        return jq.each(function() {
            ...
            $.each(param.row, function(key, value) {
                ...
                if (div.hasClass('datagrid-editable')) { //编辑状态
                    var editor = datagrid.datagrid('getEditor', {index: index, field: key});
                	...
                	if(!value&& 0!=value){   //fxq 2013-9-5 防止null值被更新为 'null'
...
//                }
//            });
//            dg.datagrid("options").queryParams.queryColumns = columnNames.substr(1);
//        });
    },
    /**
    ...
    /**
     * 导出excel。
     * @param {object} p {url:'是处理导出的Resources。',exportFileName:'导出excel的名称。'}
     * @function
    ...
    hdAdd: function(jq, row) {
        return jq.each(function() {
            ...
            if ($(this).datagrid("options").hdIsEndEdit) {
                ...
                var selectIdx = 0;
                $(this).datagrid("insertRow", {index: selectIdx, row: row})
                        .datagrid("selectRow", selectIdx)
                        .datagrid("beginEdit", selectIdx);
...
//            }
    ...
    hdRemove: function(jq, callback) {
        return jq.each(function() {
            ...
            $.messager.confirm(Resources.WB_CONFIRM, Resources.WB_REMOVE_CONFIRM, function(r) {
                if (r) {
                    dg.datagrid('cancelEdit', selectIdx)
                            .datagrid('deleteRow', selectIdx);
    ...
    hdRealRemove2: function(jq, params, callback) {
        return jq.each(function() {
            var changedData = {};
            var selectData =  $(this).datagrid("getChecked");
            ...
            var dg = $(this);
            $.messager.confirm(Resources.WB_CONFIRM, Resources.WB_REMOVE_CONFIRM2, function(r) {
                if (r) {
                    $.ajax({
                        url: url,
                        type: "POST",
     ...
     * 保存功能。
     * @function
     * @param {object} params  {url:xxx,callback:function(){}}。
