...
public interface AuthMenuRepository extends PagingAndSortingRepository<AuthMenu,String> {
    ...
    @Modifying
    @Query(value = "SELECT  t.ID, t.PARENT_ID, t.TEXT, t.URL, level    FROM ( select distinct  ID,PARENT_ID,TEXT, URL from VJ_AUTHMENU WHERE account = ?1) T START WITH t.PARENT_ID='0' CONNECT BY prior  t.id=t.PARENT_ID   ORDER BY LEVEL, parent_id",nativeQuery = true)
